...
pub fn spawn(glyph: Res<GlyphAsset>, mut commands: Commands) {
    let height = FIELD.lines().count();
    let width = FIELD.lines().next().unwrap().chars().count();
    ...
    for x in 0..map_size.x {
        for y in 0..map_size.y {
            let line = FIELD.lines().nth(y as usize).unwrap();
            ...
            let index = match char {
                '#' => {
                    ...
                    TileTextureIndex(Glyphs::FULL)
                }
                '|' => {
                    vec.push((
                        Vec3::new(x as f32 * 8.0, y as f32 * 8.0, 0.0),
                        Interactables::Goal(Teams::Player),
                        Name::from(format!("Goal {x},{y}")),
                    ));
                    TileTextureIndex(Glyphs::GOAL_NET)
                }
                'x' => {
                    vec.push((
                        Vec3::new(x as f32 * 8.0, y as f32 * 8.0, 0.0),
                        Interactables::Goal(Teams::Enemy),
                        Name::from(format!("Goal {x},{y}")),
                    ));
                    TileTextureIndex(Glyphs::GOAL_NET)
                }
                '.' => TileTextureIndex(Glyphs::MARKINGS),
                _ => TileTextureIndex(Glyphs::EMPTY),
            };
